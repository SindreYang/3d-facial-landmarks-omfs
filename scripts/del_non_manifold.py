...
for filepath in glob.iglob(f'{rootdir}*/*.obj'):
    ...
    if not os.path.exists(
        os.path.join(
            rootdir, folder_num, f'ldmks{os.path.splitext(filename)[0]}.txt'
        )
    ):
        continue
    ...
    print(f"applying filters {filepath}")
    print("sqecd")
    ...
    os.remove(os.path.join(target_dir, folder_num, f'{filename}.mtl'))
