...
def main():
    ...
    ldmks = np.load('/Volumes/Extreme SSD/MscProject/no-op/no_op_manual_labels/ldmks.pkl',
                    allow_pickle=True)  # shape (samples, landmarks + 1, 3)
    ...
    for filepath in tqdm(glob.iglob(os.path.join(rootdir, '*', '*.txt'))):
        print(filepath)
