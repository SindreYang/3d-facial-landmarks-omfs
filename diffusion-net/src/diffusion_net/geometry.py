...
def face_normals(verts, faces, normalized=True):
    ...
    return normalize(raw_normal) if normalized else raw_normal
...
def build_tangent_frames(verts, faces, normals=None):
    ...
    vert_normals = vertex_normals(verts, faces) if normals is None else normals
...
def build_grad_point_cloud(verts, frames, n_neighbors_cloud=30):
    ...
    if torch.cuda.is_available():
        ...
        edge_tangent_vecs = edge_tangent_vectors(verts, frames, edges)
        return build_grad_cpu(verts_np, torch.tensor(edges), edge_tangent_vecs)
...
def edge_tangent_vectors(verts, frames, edges):
    ...
    return torch.stack((compX, compY), dim=-1)
...
def build_grad_cpu(verts, edges, edge_tangent_vectors):    # slow
    """
...
def compute_operators(verts, faces, k_eig, normals=None):
    ...
    if k_eig > 0:
        ...
        while True:
            try:
                ...
                failcount += 1
                print(f"--- decomp failed; adding eps ===> count: {failcount}")
    ...
    if is_cloud:
        ...
        if cuda_available:
            ...
            gradX, gradY = build_grad_cuda(verts_cuda, edges_tensor_cuda, edge_tangent_vecs_cuda,
...
def populate_cache(verts, faces, k_eig, op_cache_dir=None, normals=None):
    ...
    if op_cache_dir is not None:
        ...
        while True:
            ...
            search_path = os.path.join(
                op_cache_dir, f"{hash_key_str}_{str(i_cache_search)}.npz"
            )
...
def compute_hks(evals, evecs, scales):
    ...
    return out.squeeze(0) if expand_batch else out
...
def find_knn(points_source, points_target, k, largest=False, omit_diagonal=False, method='brute'):
    ...
    if method == 'brute':
        ...
        return torch.topk(dist_mat, k=k, largest=largest, sorted=True)
    elif method == 'cpu_kd':
...
def all_pairs_geodesic_worker(verts, faces, i):
    ...
    return igl.exact_geodesic(verts, faces, sources, targets)
...
def get_all_pairs_geodesic_distance(verts_np, faces_np, geodesic_cache_dir=None):
    ...
    if geodesic_cache_dir is not None:
        ...
        while True:
            ...
            # Form the name of the file to check
            search_path = os.path.join(
                geodesic_cache_dir, f"{hash_key_str}_{str(i_cache_search)}.npz"
            )
    ...
    if not found:
        ...
        if geodesic_cache_dir is not None:
            ...
            print(f"saving geodesic distances to cache: {str(geodesic_cache_dir)}")
...
### ---- ADDED FOR LANDMARK DETECTION --- ###

def mirror(verts, ldmks):

    pcl = verts[:, :3]

    # find landmark points by max activations
    ldmk_pts = torch.argmax(ldmks, dim=1)
    ...
    # rotate point cloud to align point cloud with x axis
    d = np.dot(pcl, R.T)
